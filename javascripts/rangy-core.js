...
rangy.createModule("DomUtil", function(api, module) {
    ...
    var isChromeExt = chrome.extension !== undefined;
         
    // Perform feature tests
    if (!isChromeExt && !util.areHostMethods(document, ["createDocumentFragment", "createElement", "createTextNode"])) {
    ...
    // Gets the first non-text child of root which contains node; node can contain itself.
...
rangy.createModule("DomRange", function(api, module) {
    ...
    RangePrototype.prototype = {
        ...
        // Gorilla Patched.
        prepareSurround: function(){
            var iterator = new RangeIterator(this, true);
            var range = this.cloneRange();
            if (iterator._first && isNonTextPartiallySelected(iterator._first, this)){
                range.startContainer.parentNode.normalize();
                this.setStartBefore(iterator._first);
            }
            if (iterator._last && isNonTextPartiallySelected(iterator._last, this)){
                this.setEndAfter(iterator._last); 
            }
            iterator.detach();
        },
